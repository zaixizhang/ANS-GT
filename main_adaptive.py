...
from model_new import Graphormer, MLP
...
def get_reward(args, model, device, loader, p):
    ...
    reward[3] = torch.mean(torch.cat([i.unsqueeze(0) for i in r[3]])).cpu().numpy()/1.5
    return reward
...
def main():
    ...
    parser.add_argument('--weight_update_period', type=int, default=1000, help='epochs to update the sampling weight')
